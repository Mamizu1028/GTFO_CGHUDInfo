...
public class CGHUDInfo : Feature
{
    ...
    public override void Init()
    {
        ...
        if (Settings.ShowSlots.Count == 0)
        {
            Settings.ShowSlots = new List<HUDInfos>
            {
                HUDInfos.Health,
                HUDInfos.Infection,
            ...
            MarkSettingsDirty(Settings.ShowSlots);
        }
    }
        ...
        public static void UpdateSizeUp(PlayerAgent localPlayer)
        {
            ...
            if (localPlayer.Alive && currentResourcePackType != eResourcePackType.None)
            {
                if (Settings.AutoHideText)
                {
                    ...
                    hideInfection = currentResourcePackType != eResourcePackType.Disinfection && currentResourcePackType != eResourcePackType.Health;
        ...
        private static bool Prefix(PlaceNavMarkerOnGO __instance)
        {
            ...
            if (!hideInfection && Settings.ShowSlots.Contains(HUDInfos.Infection))
            {
                if (damageable.Infection > 0.1f)
                    sb.AppendLine($"<color=#00FFA8><size={(infectionSizeUp ? "100" : "70")}%>{Localization.GetById(2)} {damageable.Infection * 100f:N0}%</size></color>");
            }
            ...
            if (backpack != null)
            {
                ...
                if (!hideOthers && Settings.ShowSlots.Contains(HUDInfos.ResourcePack))
                {
                    ...
                    if (!flag && !Settings.HideEmptySlots)
                        sb.AppendLine($"<color=#B3B3B3>{Localization.GetById(3)}</color>");
                }
                ...
                if (!hideOthers && Settings.ShowSlots.Contains(HUDInfos.Consumable))
                {
                    ...
                    if (!flag && !Settings.HideEmptySlots)
                        sb.AppendLine($"<color=#B3B3B3>{Localization.GetById(4)}</color>");
                }
            }
            ...
            if (!hideOthers && Settings.ShowSlots.Contains(HUDInfos.BotLeader) && owner.IsBot)
            {
                ...
                if (component != null)
                {
                    ...
                    if (leader != null)
                        sb.AppendLine($"{Localization.GetById(5)} <color=#{ColorUtility.ToHtmlStringRGB(leader.Owner.PlayerColor)}>{(leader.IsLocallyOwned ? $"{Localization.GetById(6)}" : leader.Owner.NickName)}</color>");
                }
            }
    ...
    private class PlayerHudDistanceModifier : MonoBehaviour
    {
        ...
        public void UpdateNavMarkerOnPlayer(bool manualUpdate = false)
        {
            ...
            if (FocusStateManager.CurrentState == eFocusState.Freeflight)
            {
